...
select(_Config) ->
    ?assertMatch({error, _}, pgo:query(default, "select $1", [])),

    ?assertMatch(#{rows := [{null}]}, pgo:query(default, "select null")),
    ?assertMatch(#{rows := [{null}]}, pgo:query(default, "select null", [])),
    ?assertMatch(#{rows := [{null}]}, pgo:query(default, "select null")),
    ?assertMatch(#{rows := [{null}]}, pgo:query(default, "select null", [])).
...
insert_update(_Config) ->
    ...
    ?assertMatch(#{command := insert}, pgo:query(default, "insert into foo (id, some_text) values (2, 'hello again')")),
    ?assertMatch(#{}, pgo:query(default, "update foo set some_text = 'hello world' where id = 1")),
    ?assertMatch(#{}, pgo:query(default, "update foo set some_text = 'goodbye, all' where id = 3")),
    ...
    ?assertMatch(#{rows := [{<<"hello again">>}]}, pgo:query(default, "select some_text from foo where id = 2")),
    ?assertMatch(#{}, pgo:query(default, "select * from foo where id = 3")).
...
json_jsonb(_Config) ->
    ...
    #{command := insert} = pgo:query(default, "insert into tmp_b (id, a_json) values ($1, $2)",
                                     [1, <<"[{\"a\":\"foo\"},{\"b\":\"bar\"},{\"c\":\"baz\"}]">>]),
    #{command := select, rows := Rows3} = pgo:query(default, "select a_json from tmp_b where id = 1"),
    ?assertMatch([[#{<<"a">> := <<"foo">>},
                   #{<<"b">> := <<"bar">>},
...
types(_Config) ->
    ...
    ?assertMatch(#{command := insert}, pgo:query(default, "insert into types (id, an_integer, a_bigint, a_text, a_uuid, a_bytea, a_real) values ($1, $2, $3, $4, $5, $6, $7)",
    ...
    R1 = pgo:query(default, "select * from types where id = $1", [10]),
    ...
    R2 = pgo:query(default, "select * from types where id = $1", [11]),
    ...
    R3 = pgo:query(default, "select a_text from types where id IN ($1, $2) order by id", [17, 18]),
