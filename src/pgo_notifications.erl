...
-spec listen(pid(), channel()) -> {ok, reference()}.
listen(Pid, Channel) when is_binary(Channel) ->
    gen_statem:call(Pid, {listen, Channel}).

-spec unlisten(pid(), reference()) -> ok.
unlisten(Pid, Ref) ->
    gen_statem:call(Pid, {unlisten, Ref}).
